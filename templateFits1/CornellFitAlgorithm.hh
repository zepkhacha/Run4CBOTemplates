        ...
        if (pBinCBO){
            ...
            if (desiredCalo==0){
                ...
                x += exp(-2.*time/par[5]         )*(par[10]*cos(2*par[6]*cbo(time, par[24])*time) 
                    ...
                    double alpha_CBO = (alpha_CBO_TF1[desiredCalo-1]).Eval(time);
                    double beta_CBO = (beta_CBO_TF1 [desiredCalo-1]).Eval(time);
                    double alpha_2CBO = (alpha_2CBO_TF1[desiredCalo-1]).Eval(time);
                    double beta_2CBO = (beta_2CBO_TF1 [desiredCalo-1]).Eval(time);
                    double alpha_y = (alpha_y_TF1[desiredCalo-1]).Eval(time);
                    double beta_y = (beta_y_TF1 [desiredCalo-1]).Eval(time);
                    double alpha_vw = (alpha_vw_TF1[desiredCalo-1]).Eval(time);
                    double beta_vw = (beta_vw_TF1 [desiredCalo-1]).Eval(time);
