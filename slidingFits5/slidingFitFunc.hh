...
double calcnu(double par[], double i){
    ...
    // x-y cross term 
    double term_vw_m_CBO = (par[24]*cos(par[25]*time) + par[26]*sin(par[25]*time));
    double term_vw_p_CBO = (par[27]*cos(par[28]*time) + par[29]*sin(par[28]*time));

    // now add LM 
    double term_LM = (1.0 - par[16]*lambda->GetBinContent(i));

    return term_5param * term_LM * ( term_x + term_y + term_vw_m_CBO + term_vw_p_CBO ); 
