...
std::vector<TF1> alpha_vw_m_cbo_TF1;
std::vector<TF1> beta_vw_m_cbo_TF1;
std::vector<TF1> alpha_vw_p_cbo_TF1;
std::vector<TF1> beta_vw_p_cbo_TF1;
...
double calcnu(double *dim, double *par){ // dim[0] = bin number
    ...
    for (int b=calcnuRangeLower; b<=calcnuRangeUpper; b++){
        ...
        double beta_phi  = 0.0;
        double alpha_vw_m_cbo  = 0.0;
        double beta_vw_m_cbo   = 0.0;
        double alpha_vw_p_cbo  = 0.0;
        double beta_vw_p_cbo   = 0.0;
        ...
        for (int caloNum=0; caloNum<24; caloNum++){
            ...
            alpha_vw_m_cbo   += caloWeights[caloNum]*(alpha_vw_m_cbo_TF1  [desiredCalo]).Eval(time);
            beta_vw_m_cbo    += caloWeights[caloNum]*(beta_vw_m_cbo_TF1   [desiredCalo]).Eval(time);
            alpha_vw_p_cbo   += caloWeights[caloNum]*(alpha_vw_p_cbo_TF1  [desiredCalo]).Eval(time);
            beta_vw_p_cbo    += caloWeights[caloNum]*(beta_vw_p_cbo_TF1   [desiredCalo]).Eval(time);
        ...
        double term_vw_m_cbo = (par[22]*alpha_vw_m_cbo*cos(time*par[26])+par[23]*beta_vw_m_cbo*sin(time*par[26]));
        double term_vw_p_cbo = (par[29]*alpha_vw_p_cbo*cos(time*par[32])+par[30]*beta_vw_p_cbo*sin(time*par[32]));
        binNu *= x*y +term_vw_m_cbo+term_vw_p_cbo; 
