...
int main(int argc, char* argv[]){
    ...
    if (format.is_open()) {
        ...
        while (getline(format, line)){
            ...
            }else if    (line.find("iterationNo") != std::string::npos) {
                iterationNo = atoi(line.substr(datIndex,100).c_str());
            }
        }
    } 
    format.close();
    ...
    double alpha_vw_m_cbo, beta_vw_m_cbo, w_vw_m_cbo;
    double alpha_vw_m_cbo_err, beta_vw_m_cbo_err, w_vw_m_cbo_err;
    ...
    fitresults->Branch("iteration", &iterationNo);
    ...
    // X-Y cross terms
    fitresults->Branch("alpha_vw_m_cbo", &alpha_vw_m_cbo);
    fitresults->Branch("beta_vw_m_cbo" , &beta_vw_m_cbo);
    fitresults->Branch("w_vw_m_cbo" , &w_vw_m_cbo);
    fitresults->Branch("alpha_vw_m_cbo_err", &alpha_vw_m_cbo_err);
    fitresults->Branch("beta_vw_m_cbo_err" , &beta_vw_m_cbo_err);
    fitresults->Branch("w_vw_m_cbo_err" , &w_vw_m_cbo_err);

    fitresults->Branch("alpha_vw_p_cbo", &alpha_vw_p_cbo);
    fitresults->Branch("beta_vw_p_cbo" , &beta_vw_p_cbo);
    fitresults->Branch("w_vw_p_cbo" , &w_vw_p_cbo);
    fitresults->Branch("alpha_vw_p_cbo_err", &alpha_vw_p_cbo_err);
    fitresults->Branch("beta_vw_p_cbo_err" , &beta_vw_p_cbo_err);
    fitresults->Branch("w_vw_p_cbo_err" , &w_vw_p_cbo_err);
    ...
    minimizer.DefineParameter(25, "w_vw_m_cbo", 12.075412, 0.0, 0, 0);
    minimizer.DefineParameter(26, "beta_vw_m_cbo", 0.0, 0.001, 0, 0);

    // now adding in x-y cross term (VW + CBO)
    minimizer.DefineParameter(27, "alpha_vw_p_cbo", 0.0, 0.001, 0, 0);
    minimizer.DefineParameter(28, "w_vw_p_cbo", 12.075412, 0.0, 0, 0);
    ...
    // set cross terms
    alpha_vw_m_cbo = par[24];
    w_vw_m_cbo     = par[25];
    beta_vw_m_cbo  = par[26];

    alpha_vw_m_cbo_err = sqrt(-errorplus[24]*errorminus[24]);
    w_vw_m_cbo_err     = sqrt(-errorplus[25]*errorminus[25]);
    beta_vw_m_cbo_err  = sqrt(-errorplus[26]*errorminus[26]);

    alpha_vw_p_cbo = par[27];
    w_vw_p_cbo     = par[28];
    beta_vw_p_cbo  = par[29];

    alpha_vw_p_cbo_err = sqrt(-errorplus[27]*errorminus[27]);
    w_vw_p_cbo_err     = sqrt(-errorplus[28]*errorminus[28]);
    beta_vw_p_cbo_err  = sqrt(-errorplus[29]*errorminus[29]);
