...
void minuitFunction( __attribute__((unused)) int& nDim,
    ...
    for(int i=fitrangelow; i<=fitrangehigh; i++){
        ...
        /*
           ...
           */
        //TSpline3 For standard error computations
        //TSpline3result += chisq;
    }
    ...
    if(includeMopTerm and constrainMop){
        //TSpline3double error = (par[27] * 2.1969811 * 0.004) / (2.1969811 * frlifetime);
...
int main(int argc, char* argv[]){
    ...
    for (int i=0; i<25; i++){
        TSpline3* temp_alpha_CBO = (TSpline3*)f_alpha_CBO->Get(Form("calo%i_alpha_CBO_spline", desiredCalo));
        TSpline3* temp_beta_CBO  = (TSpline3*)f_beta_CBO ->Get(Form("calo%i_beta_CBO_spline" , desiredCalo));
        TSpline3* temp_alpha_2CBO = (TSpline3*)f_alpha_2CBO->Get(Form("calo%i_alpha_2CBO_spline", desiredCalo));
        TSpline3* temp_beta_2CBO  = (TSpline3*)f_beta_2CBO ->Get(Form("calo%i_beta_2CBO_spline" , desiredCalo));
        TSpline3* temp_alpha_y = (TSpline3*)f_alpha_y->Get(Form("calo%i_alpha_y_spline", desiredCalo));
        TSpline3* temp_beta_y  = (TSpline3*)f_beta_y ->Get(Form("calo%i_beta_y_spline" , desiredCalo));
        TSpline3* temp_alpha_vw = (TSpline3*)f_alpha_vw->Get(Form("calo%i_alpha_vw_spline", desiredCalo));
        TSpline3* temp_beta_vw  = (TSpline3*)f_beta_vw ->Get(Form("calo%i_beta_vw_spline" , desiredCalo));
        TSpline3* temp_alpha_A0 = (TSpline3*)f_alpha_A0->Get(Form("calo%i_alpha_A0_spline", desiredCalo));
        TSpline3* temp_beta_A0  = (TSpline3*)f_beta_A0 ->Get(Form("calo%i_beta_A0_spline" , desiredCalo));
        TSpline3* temp_alpha_phi = (TSpline3*)f_alpha_phi->Get(Form("calo%i_alpha_phi_spline", desiredCalo));
        TSpline3* temp_beta_phi  = (TSpline3*)f_beta_phi ->Get(Form("calo%i_beta_phi_spline" , desiredCalo));
        TSpline3* temp_alpha_xy = (TSpline3*)f_alpha_xy->Get(Form("calo%i_alpha_xy_spline", desiredCalo));
        TSpline3* temp_beta_xy  = (TSpline3*)f_beta_xy ->Get(Form("calo%i_beta_xy_spline" , desiredCalo));

        alpha_CBO_spline.push_back(*temp_alpha_CBO);
        beta_CBO_spline .push_back(*temp_beta_CBO); 
        alpha_2CBO_spline.push_back(*temp_alpha_2CBO);
        beta_2CBO_spline .push_back(*temp_beta_2CBO); 
        alpha_y_spline.push_back(*temp_alpha_y);
        beta_y_spline .push_back(*temp_beta_y); 
        alpha_vw_spline.push_back(*temp_alpha_vw);
        beta_vw_spline .push_back(*temp_beta_vw); 
        alpha_A0_spline.push_back(*temp_alpha_A0);
        beta_A0_spline .push_back(*temp_beta_A0); 
        alpha_phi_spline.push_back(*temp_alpha_phi);
        beta_phi_spline .push_back(*temp_beta_phi); 
        alpha_xy_spline.push_back(*temp_alpha_xy);
        beta_xy_spline .push_back(*temp_beta_xy); 
    }
    ...
    for(int seed=desiredSeed; seed<desiredSeed+1; seed++){
        ...
        if (pBinCBO){
            ...
            minimizer.DefineParameter(7, "A_CNx1",  1.0, 0.0, -2.0, 2.0); // USE TO SCALE ALPHA/BETA
            minimizer.DefineParameter(8, "A_SNx1",  1.0, 0.0, -1.0, 1.0); // SUB WITH TF1
            minimizer.DefineParameter(10,"A_CNx2",  1.0, 0.0, -1.0, 1.0);
            minimizer.DefineParameter(11,"A_SNx2",  1.0, 0.0, -1.0, 1.0);
            minimizer.DefineParameter(14,"A_CNy1",  1.0, 0.0, -1.0, 1.0);
            minimizer.DefineParameter(15,"A_SNy1",  1.0, 0.0, -1.0, 1.0);
            minimizer.DefineParameter(18,"A_CNy2",  1.0, 0.0, -1.0, 1.0);
