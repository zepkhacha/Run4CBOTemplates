...
std::vector<TSpline3> alpha_CBO_spline;
std::vector<TSpline3>  beta_CBO_spline;
std::vector<TSpline3> alpha_2CBO_spline;
std::vector<TSpline3>  beta_2CBO_spline;
std::vector<TSpline3> alpha_y_spline;
std::vector<TSpline3>  beta_y_spline;
std::vector<TSpline3> alpha_vw_spline;
std::vector<TSpline3>  beta_vw_spline;
std::vector<TSpline3> alpha_xy_spline;
std::vector<TSpline3>  beta_xy_spline;
std::vector<TSpline3> alpha_phi_spline;
std::vector<TSpline3>  beta_phi_spline;
std::vector<TSpline3> alpha_A0_spline;
std::vector<TSpline3>  beta_A0_spline;
...
double calcnu(double *dim, double *par){ // dim[0] = bin number
    ...
    for (int b=calcnuRangeLower; b<=calcnuRangeUpper; b++){
        ...
        for (int caloNum=0; caloNum<24; caloNum++){
            ...
            alpha_CBO  += caloWeights[caloNum]*(alpha_CBO_spline [desiredCalo]).Eval(time);
            beta_CBO   += caloWeights[caloNum]*(beta_CBO_spline  [desiredCalo]).Eval(time);
            alpha_2CBO += caloWeights[caloNum]*(alpha_2CBO_spline[desiredCalo]).Eval(time);
            beta_2CBO  += caloWeights[caloNum]*(beta_2CBO_spline [desiredCalo]).Eval(time);
            alpha_y    += caloWeights[caloNum]*(alpha_y_spline   [desiredCalo]).Eval(time);
            beta_y     += caloWeights[caloNum]*(beta_y_spline    [desiredCalo]).Eval(time);
            alpha_vw   += caloWeights[caloNum]*(alpha_vw_spline  [desiredCalo]).Eval(time);
            beta_vw    += caloWeights[caloNum]*(beta_vw_spline   [desiredCalo]).Eval(time);
            alpha_A0   += caloWeights[caloNum]*(alpha_A0_spline  [desiredCalo]).Eval(time);
            beta_A0    += caloWeights[caloNum]*(beta_A0_spline   [desiredCalo]).Eval(time);
            alpha_phi  += caloWeights[caloNum]*(alpha_phi_spline [desiredCalo]).Eval(time);
            beta_phi   += caloWeights[caloNum]*(beta_phi_spline  [desiredCalo]).Eval(time);
            alpha_xy   += caloWeights[caloNum]*(alpha_xy_spline  [desiredCalo]).Eval(time);
            beta_xy    += caloWeights[caloNum]*(beta_xy_spline   [desiredCalo]).Eval(time);
